...
<header class="text-center">
    <div class="fs-3 fw-bold text-uppercase">
        DEDUCTION N° {{ $deduction->reference }}
    </div>
</header>
        ...
        <div>
            ...
            <p class="m-0">
                <strong>Nom complet :</strong> <span
                    class="text-uppercase fw-bold fs-6">{{ $deduction->creator?->nom_utilisateur ?? '—' }}</span>
            </p>
            <p class="m-0">
                <strong>Email :</strong> <span class="fw-bold fs-6">{{ $deduction->creator?->email ?? '—' }}</span>
            </p>
        </div>
    @endif

    @if($deduction->validator)
        <div>
            <div class="fs-6 fw-bold">Valider par :</div>
            <p class="m-0">
                <strong>Nom complet :</strong> <span
                    class="text-uppercase fw-bold fs-6">{{ $deduction->validator?->nom_utilisateur ?? '—' }}</span>
            </p>
            <p class="m-0">
                <strong>Email :</strong> <span class="fw-bold fs-6">{{ $deduction->validator?->email ?? '—' }}</span>
            </p>
        </div>
    @endif
</div>
        ...
        <div>
            <p class="m-0">
                ...
                <span class="text-capitalize fw-bold">
                    {{ optional($deduction->warehouse)->name ?? '---' }}
                </span>
            </p>
        </div>
</div>
...
<div class="d-flex justify-content-center mt-2">
    <table class="table table-bordered border-black table-striped" style="font-size: 11px;">
        ...
        <tbody>
        @foreach($deduction->items as $index => $item)
            <tr>
                <td>{{ $index + 1 }}</td>
...
<div class="d-flex align-content-center mb-3" style="font-size: 13px; gap: 2rem;">
    <p class="m-0">
        ...
        <span class="text-capitalize">{{ \App\Enums\StocksDeductionsStatus::safeLabel($deduction->status) }}</span>
    </p>
</div>
...
<div class="text-end mt-3" style="font-size: 13px;">
    <p>
        Fait le
        {{ optional($deduction->created_at)->format('d/m/Y H:i') ?? '—' }}
    </p>
</div>
