...
<header class="text-center">
    <div class="fs-3 fw-bold text-uppercase">
        INVENTAIRE DE STOCKS DE L'ENTREPÔT {{ $warehouse->name }}
    </div>
</header>
...
<div class="d-flex justify-content-center mt-2">
    <table class="table table-bordered table-striped text-center border-black" style="font-size: 12px;">
        ...
        <tr>
            ...
            <th style="width: 10%;">Qté en stock</th>
            <th style="width: 15%;">Date création</th>
            <th style="width: 15%;">Crée par</th>
            <th style="width: 10%;">Mise à jour le</th>
            <th style="width: 10%;">Mise à jour par</th>
        </tr>
        </thead>
        ...
        @foreach ($product_points as $index => $item)
            <tr>
                ...
                <td>{{ $item->creator->nom_utilisateur ?? '-' }}</td>
                <td>{{ optional($item->updated_at)->format('d/m/Y H:i') }}</td>
                <td>{{ $item->updater->nom_utilisateur ?? '-' }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
