...
{
    private function checkStockComplement(
        $productUuid,
        $warehouseUuid,
        $quantity,
        $orderUuid = null,
        ...
        $reservedStock = (float) ComplementVirtualTemp::where('product_uuid', $productUuid)
        ...
        $MenuStocksreservedStock = (float) MenuVirtualTemp::where('product_uuid', $productUuid)
            ->where('status', 'pending')
            ->where('type', '!=', 'not_used')
            ->when($reservationUuid, function ($q) use ($reservationUuid) {
                $q->where(function ($sub) use ($reservationUuid) {
        ...
        $availableStock = max(0, $realStock - $reservedStock - $MenuStocksreservedStock);
    ...
    public function check_stock_form_complement(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'menu_quantity' => ['required', 'integer', 'min:1'],
            ...
            $menuQty = (int) $validated['menu_quantity'];

            Log::info('CHECK COMPLEMENT STOCK START', [
                'reservation_uuid' => $reservationUuid,
                'menu_uuid' => $menuUuid,
                'payload' => $validated,
            ]);
            ...
            $cartLineUuids = collect($validated['complements'])
                ->pluck('cart_line_uuid')
                ->unique();

            ComplementVirtualTemp::where('reservation_uuid', $reservationUuid)
                ->whereIn('cart_line_uuid', $cartLineUuids)
                ->where('status', 'pending')
                ->forceDelete();

            ComplementVirtualTempsBackup::where('reservation_uuid', $reservationUuid)
                ->whereIn('cart_line_uuid', $cartLineUuids)
                ->where('status', 'pending')
                ->forceDelete();
                ...
                foreach ($compositions[$complementUuid] as $composition) {
                    ...
                    foreach ($composition->items as $item) {
                        ...
                        try {
                            ...
                            Log::error('STOCK ERROR COMPLEMENT', [
            ...
            if (!empty($stockErrors)) {
                return response()->json([
                    'status' => 'error',
                    'message' => collect($stockErrors)->pluck('error')->implode(' | '),
                    'details' => $stockErrors
                ], 422);
            }


            DB::transaction(function () use ($validated, $auth, $reservationUuid, $menuUuid, $menuQty, $compositions) {
                    ...
                    foreach ($compositions[$complementUuid] as $composition) {
                        foreach ($composition->items as $item) {
                            ...
                            $data = [
                                ...
                                'menu_quantity' => $menuQty,
                                'quantity_used' => $totalUsed,
                                'updated_by' => $auth->id,
                                'created_by' => $auth->id,
                                'last_activity_at' => now(),
                            ];
                            ComplementVirtualTemp::create($data);
                            ComplementVirtualTempsBackup::create($data);
                        }
                    }
                }
            });
            ...
            Log::warning('VALIDATION ERROR COMPLEMENT STOCK', [
                'errors' => $e->errors()
            ]);
            ...
            Log::error('CHECK COMPLEMENT STOCK FATAL ERROR', [
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'reservation_uuid' => $reservationUuid,
    ...
    public function getSavedComplements(Request $request)
    {
        $validated = $request->validate([
            'cart_line_uuid'              => ['required', 'uuid'],
            'reservation_uuid'            => ['nullable', 'uuid'],
            'order_menu_restaurant_uuid'  => ['nullable', 'uuid'],
        ]);

        try {
            $query = ComplementVirtualTemp::where('cart_line_uuid', $validated['cart_line_uuid'])
                ->where('status', 'pending');

            if (!empty($validated['order_menu_restaurant_uuid'])) {
                $query->where('order_menu_restaurant_uuid', $validated['order_menu_restaurant_uuid']);
            } elseif (!empty($validated['reservation_uuid'])) {
                $query->where('reservation_uuid', $validated['reservation_uuid']);
            } else {
                return response()->json([
                    'status' => 'error',
                    'message' => 'reservation_uuid ou order_menu_restaurant_uuid requis.'
                ], 422);
            }

            $savedTemps = $query->get();
            ...
            $formattedComplements = $savedTemps->groupBy('complement_uuid')
                ->map(function ($items, $complementUuid) {
                    ...
                    return [
                        ...
                        'cart_line_uuid'        => $firstItem->cart_line_uuid,
                    ];
                })->values();
            ...
            Log::error('ERROR GET SAVED COMPLEMENTS LIGHT', [
                'message' => $e->getMessage(),
                'payload' => $validated
            ]);
    ...
    public function check_stock_form_complement_to_new(Request $request)
    {
        ...
        try {
            ...
            $complementUuids = $normalizedComplements->pluck('uuid')->unique()->toArray();
            $cartLineUuids   = $normalizedComplements->pluck('cart_line_uuid')->unique()->toArray();

            $primaryCartLineUuid = $cartLineUuids[0] ?? $defaultCartLineUuid;
                ...
                foreach ($compositions[$complementUuid] as $composition) {
                    ...
                    foreach ($composition->items as $item) {
                        ...
                        try {
                            ...
                            if (preg_match('/Disponible\s*:\s*(\d+)/i', $msg, $matches)) {
                            ...
                            $stockErrors[] = [
                                ...
                                'quantity_available' => $available ?? 0,
                                'error'              => $msg
                            ];
                        }
                    }
            ...
            return response()->json([
                ...
                'cart_line_uuid'     => $primaryCartLineUuid,
                'reservation_uuid'   => $reservationUuid,
                'expires_in_minutes' => method_exists($this, 'getLogoutMinutes') ? $this->getLogoutMinutes() : 30,
            ]);
    ...
    public function updateActivity(Request $request)
    {
        ...
        ComplementVirtualTemp::where('reservation_uuid', $request->reservation_uuid)
            ->update([
                'last_activity_at' => now()
            ]);

        ComplementVirtualTempsBackup::where('reservation_uuid', $request->reservation_uuid)
            ->update([
                'last_activity_at' => now()
            ]);
    ...
    public function removeReservationItem(Request $request)
    {
        ...
        $deleted = MenuVirtualTemp::where('reservation_uuid', $validated['reservation_uuid'])
            ->where('cart_line_uuid', $validated['cart_line_uuid'])
            ->forceDelete();

        $deleted_complements = ComplementVirtualTemp::where('reservation_uuid', $validated['reservation_uuid'])
            ->where('cart_line_uuid', $validated['cart_line_uuid'])
            ->forceDelete();

        $deleted_complements_backups = ComplementVirtualTempsBackup::where('reservation_uuid', $validated['reservation_uuid'])
            ->where('cart_line_uuid', $validated['cart_line_uuid'])
            ->forceDelete();
        ...
        return response()->json([
            ...
            'deleted_complements' => $deleted_complements,
            'deleted_complements_backups' => $deleted_complements_backups
        ]);
    }
    ...
    public function checkStockOnly(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'menus.*.cart_line_uuid' => ['required', 'uuid'],
            ]);
            ...
            foreach ($validated['menus'] as $MenusInput) {
                ...
                MenuVirtualTemp::where('reservation_uuid', $reservationUuid)
                    ->where('cart_line_uuid', $MenusInput['cart_line_uuid'])
                    ->where('status', 'pending')
                    ->forceDelete();
            }
            ...
            foreach ($validated['menus'] as $menuInput) {
                ...
                foreach ($menuItems[$menuInput['menus_restaurant_uuid']] ?? [] as $item) {
                    ...
                    MenuVirtualTemp::create([
                        'cart_line_uuid' => $menuInput['cart_line_uuid'],
                        'reservation_uuid' => $reservationUuid,
    ...
    public function forceReleaseStock(Request $request)
    {
        ...
        ComplementVirtualTemp::where('reservation_uuid', $request->reservation_uuid)->forceDelete();
        ComplementVirtualTempsBackup::where('reservation_uuid', $request->reservation_uuid)->forceDelete();
        return response()->json([
            'status' => 'success',
    ...
    public function forceReleaseOrderMenuRestaurant(Request $request)
    {
        ...
        ComplementVirtualTemp::where('order_menu_restaurant_uuid', $request->order_menu_restaurant_uuid)
            ->whereIn('type', ['editing', 'initial'])
            ->forceDelete();

        ComplementVirtualTempsBackup::where('order_menu_restaurant_uuid', $request->order_menu_restaurant_uuid)
            ->whereIn('type', ['editing', 'initial'])
            ->forceDelete();
    ...
    public function checkStockByOrder(Request $request)
    {
        ...
        foreach ($validated['menus'] as $menuInput) {
            ...
            foreach ($menuItems[$menuInput['menus_restaurant_uuid']] ?? [] as $item) {
                ...
                $ComplementreservedStock = (float) ComplementVirtualTemp::where('product_uuid', $item->product_uuid)
                    ->where('status', 'pending')
                    ->where('type', '!=', 'not_used')
                    ->when($orderUuid, function ($q) use ($orderUuid) {
                        $q->where(function ($sub) use ($orderUuid) {
                ...
                $availableStock = max(0, $realStock - $reservedStock - $ComplementreservedStock);
    ...
    public function cancelRervationsAfterValidation(Request $request)
    {
        ...
        DB::transaction(function () use ($orderUuid, $order) {
            ...
            $complements = ComplementVirtualTempsBackup::where('order_menu_restaurant_uuid', $orderUuid)
                ->where(function ($query) {
                    $query->whereIn('type', ['initial', 'editing', 'not_used'])
                        ->orWhereNull('reservation_uuid');
                })
    ...
    public function cancelCurrentRervations(Request $request)
    {
        ...
        ComplementVirtualTemp::where('reservation_uuid', $reservationUuid)->where('type', 'initial')
            ->whereNull('order_menu_restaurant_uuid')->forceDelete();

        ComplementVirtualTempsBackup::where('reservation_uuid', $reservationUuid)->where('type', 'initial')
            ->whereNull('order_menu_restaurant_uuid')->forceDelete();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $affectedComplement = \DB::table('complement_virtual_temps')
                ->where('reservation_uuid', $reservationUuid)
                ->whereNull('order_menu_restaurant_uuid')
                ->update([
                    'order_menu_restaurant_uuid' => $order->uuid,
            ...
            $affectedComplementBackup = \DB::table('complement_virtual_temps_backup')
                ->where('reservation_uuid', $reservationUuid)
                ->whereNull('order_menu_restaurant_uuid')
                ->update([
                    'order_menu_restaurant_uuid' => $order->uuid,
    ...
    public function addItemsToOrder(Request $request, string $uuid)
    {
        ...
        try {
            ...
            if (!empty($menus)) {
                ...
                foreach ($menus as $m) {
                    ...
                    if ($existingItem) {
                        ...
                        if ($newQty == $oldQty && !$isRejectedGroup && !$isDelivered && !$isTransferred && !$isInPreparation) {
                            $this->syncItemComplements($orderItem, $menu, $order, $m, $auth);
                            continue;
                        }
                ...
                $this->backupVirtualComplements($order, $auth);
            }
    ...
    public function index(Request $request)
    {
        ...
        if (
            ...
            !$auth->can('view_kitchen_and_bar_orders')
        ) {
            ...
            $query->where(function ($q) use ($auth, $roleIds) {
                ...
                if ($auth->can('view_kitchen_orders')) {
                    $hasVisibilityPermission
                        ...
                        : $q->whereNotNull('kitchen_user_id');
        ...
        return DB::transaction(function () use ($uuid, $auth) {
            ...
            foreach ($order->items as $item) {
                ...
                foreach ($virtualComplements as $vComp) {
                    ...
                    $productPointComp = ProductPoint::where('produit_uuid', $vComp->product_uuid)
                        ->where('point_uuid', $targetWarehouseUuid)
                        ->lockForUpdate()
                        ->first();
                    ...
                    $productPointComp->update([
                        'quantity'   => $productPointComp->quantity - $qtyComp,
                        'updated_by' => $auth->id
                    ]);
    ...
    public function validateMenusForOrder(Request $request, string $uuid)
    {
        ...
        try {
            ...
            $notificationStatus = $allFinished
    ...
    public function validateAndDeductStockMenus(Request $request ,string $orderUuid)
    {
        ...
        try {
            ...
            $allDelivered = collect($order->items)->every(
                fn ($i) => $i->status === OrderMenuRestaurantItemStatus::DELIVERED->value
            );
            $hasPartial = collect($order->items)->contains(
                fn ($i) => $i->status === OrderMenuRestaurantItemStatus::PARTIAL_DELIVERED->value
            );

            if ($allDelivered) {
                $notificationStatus = MenuOrderStatus::DELIVERED->value;
                $message = "Les menus de la commande {$order->code} ont été servis avec succès.";
            } elseif ($hasPartial) {
                $notificationStatus = MenuOrderStatus::PARTIAL_DELIVERED->value;
                $message = "Les menus de la commande {$order->code} ont été servis partiellement.";
            } else {
                $notificationStatus = MenuOrderStatus::TRANSFERRED->value;
                $message = "Les menus de la commande {$order->code} sont en cours de préparation.";
            }
            \App\Models\OrderNotification::createOrUpdateNotification(
                $order->uuid,
                $notificationStatus,
    ...
    public function validateAndDeductStockDrinks(Request $request, string $orderUuid)
    {
        ...
        try {
            ...
            $allDelivered = collect($order->drinks)->every(
                fn ($d) => $d->status === OrderMenuRestaurantItemStatus::DELIVERED->value
            );

            $hasPartial = collect($order->drinks)->contains(
                fn ($d) => $d->status === OrderMenuRestaurantItemStatus::PARTIAL_DELIVERED->value
            );

            if ($allDelivered) {
                $notificationStatus = MenuOrderStatus::DELIVERED->value;
                $message = "Les boissons de la commande {$order->code} ont été servies avec succès.";
            } elseif ($hasPartial) {
                $notificationStatus = MenuOrderStatus::PARTIAL_DELIVERED->value;
                $message = "Les boissons de la commande {$order->code} ont été servies partiellement.";
            } else {
                $notificationStatus = MenuOrderStatus::TRANSFERRED->value;
                $message = "Les boissons de la commande {$order->code} sont en cours de transfert.";
            }
        ...
        return DB::transaction(function () use ($validated, $auth, $order, $priorityStatuses) {
            ...
            foreach ($validated['items'] as $data) {
                ...
                if ($item->complements && $item->complements->count() > 0) {
                    foreach ($item->complements as $orderComplement) {

                        $complementUuid = $orderComplement->configuration_complement_uuid;
                        $compRecords = \App\Models\ComplementComposition::with('items')
                            ->where('commplements_restaurant_uuid', $complementUuid)
                            ->get();

                        foreach ($compRecords as $composition) {
                            foreach ($composition->items as $compItem) {
                                $totalComplementDefectiveUsed = $qtyToDefect * $compItem->quantity_used;
                                $virtualComplement = ComplementVirtualTemp::firstOrCreate(
                                    [
                                        'order_menu_restaurant_uuid' => $order->uuid,
                                        'menu_uuid'                  => $item->menus_restaurant_uuid,
                                        'complement_uuid'            => $complementUuid,
                                        'product_uuid'               => $compItem->product_uuid,
                                        'reservation_uuid'           => $order->reservation_uuid,
                                        'type'                       => 'initial',
                                    ],
                                    [
                                        'cart_line_uuid'        => $orderComplement->cart_line_uuid ?? (string) \Illuminate\Support\Str::uuid(),
                                        'status'                => 'pending',
                                        'quantity'              => 0,
                                        'menu_quantity'         => $item->quantity_exactly,
                                        'quantity_used'         => 0,
                                        'quantity_in_defective' => 0,
                                        'created_by'            => $auth->id,
                                        'updated_by'            => $auth->id,
                                ...
                                $virtualComplement->increment('quantity_in_defective', $totalComplementDefectiveUsed);
                            }
                        }
                    }
                }
            ...
            foreach ($validated['items'] as $data) {
                ...
                if ($item->complements && $item->complements->count() > 0) {
                    foreach ($item->complements as $orderComplement) {
                        ...
                        foreach ($compRecords as $composition) {
                            foreach ($composition->items as $compItem) {
                                $totalComplementRestoredUsed = $qtyToRestore * $compItem->quantity_used;

                                $virtualComplement = ComplementVirtualTemp::where([
                                    'order_menu_restaurant_uuid' => $order->uuid,
                                    'menu_uuid'                  => $item->menus_restaurant_uuid,
                                    'complement_uuid'            => $complementUuid,
                                    'product_uuid'               => $compItem->product_uuid,
                                ...
                                if ($virtualComplement) {
                                    $virtualComplement->decrement('quantity_in_defective', $totalComplementRestoredUsed);
                                    if ($virtualComplement->fresh()->quantity_in_defective < 0) {
                                        $virtualComplement->update(['quantity_in_defective' => 0]);
                                    }
                                    $virtualComplement->update([
                                        'updated_by'       => $auth->id,
                                        'last_activity_at' => now()
                                    ]);
                                }
        ...
        return DB::transaction(function () use ($request, $order, $auth) {
            ...
            foreach ($request->items as $data) {
                ...
                $virtualComplements = ComplementVirtualTemp::where([
                    ...
                    'menu_uuid'                  => $item->menus_restaurant_uuid,
                    'type'                       => 'initial'
                ])->where('quantity_in_defective', '>', 0)->lockForUpdate()->get();

                foreach ($virtualComplements as $vComp) {
                    $toDeductComp = $vComp->quantity_in_defective;
                    $composition = \App\Models\ComplementComposition::where('commplements_restaurant_uuid', $vComp->complement_uuid)
                        ->first();
                    $targetWarehouseUuid = $composition ? $composition->warehouse_uuid : $warehouse->uuid;
                    ...
                    if (!$productPointComp) {
                        throw new \Exception("Stock introuvable dans l'entrepôt ciblé pour l'ingrédient du complément {$vComp->product_uuid}");
                    }

                    $productPointComp->decrement('quantity', $toDeductComp);

                    $vComp->update([
                        'quantity_used'         => max(0, $vComp->quantity_used - $toDeductComp),
                        'quantity_in_defective' => max(0, $vComp->quantity_in_defective - $toDeductComp),
                        'menu_quantity' => max(0, $vComp->menu_quantity - $toDeductComp),
                        'updated_by'            => $auth->id,
                        'last_activity_at'      => now()
                    ]);
                }
