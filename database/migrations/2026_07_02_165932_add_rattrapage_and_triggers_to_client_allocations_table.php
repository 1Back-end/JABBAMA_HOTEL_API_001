...
return new class extends Migration
{
    ...
    {
        // =================================================================
        // 0. RATTRAPAGE DES DONNÉES EXISTANTES (Sécurisé avec INSERT IGNORE)
        // =================================================================

        // Récupérer l'historique des partenaires
        DB::statement("
            INSERT IGNORE INTO client_allocations (uuid, source_type, source_uuid, client_name, amount_allocated, amount_allocated_total, created_at, updated_at)
            SELECT UUID(), 'partner', uuid, full_name, amount_allocated, amount_allocated_total, created_at, updated_at
            FROM restaurant_partners
        ");

        // Récupérer l'historique des clients libres
        DB::statement("
            INSERT IGNORE INTO client_allocations (uuid, source_type, source_uuid, client_name, amount_allocated, amount_allocated_total, created_at, updated_at)
            SELECT UUID(), 'free_client', uuid, full_name, amount_allocated, amount_allocated_total, created_at, updated_at
            FROM free_clients_restaurants
        ");

        // Récupérer l'historique des commandes (uniquement celles avec une allocation > 0)
        DB::statement("
            INSERT IGNORE INTO client_allocations (uuid, source_type, source_uuid, client_name, amount_allocated, amount_allocated_total, created_at, updated_at)
            SELECT UUID(), 'order', uuid, full_name, amount_allocated, amount_allocated, created_at, updated_at
    ...
    public function down(): void
    {
        ...
        // Nettoyage de la table centrale en cas de rollback
        DB::table('client_allocations')->truncate();
    }
};
